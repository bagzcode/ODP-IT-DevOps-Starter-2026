...
document.getElementById("getAccountBtn").onclick = () =>
  api(
    `/api/v1/accounts/${encodeURIComponent(accountInput.value.trim())}`,
  ).catch(() => {});
document.getElementById("mutationsBtn").onclick = () =>
  api(
    `/api/v1/accounts/${encodeURIComponent(accountInput.value.trim())}/mutationz`,
  ).catch(() => {});
...
document.getElementById("transactBtn").onclick = () =>
  api(
    `/api/v1/accounts/${encodeURIComponent(accountInput.value.trim())}/transact`,
    {
      method: "POST",
      body: JSON.stringify({
